...
def load_matrix(name: str) -> sparse.csr_matrix:
    ...
    if not path.exists():
        fail(f"Missing feature matrix: {path}")
...
def main() -> None:
    print("=" * 70)
    print("XGBOOST FULL MULTI-RELATIONAL GRAPH MODEL")
    print("=" * 70)
    ...
    model_path = (
        MODEL_DIR
        / "xgboost_multigraph_enhanced.json"
    )

    model.save_model(model_path)

    metrics = {
        "model": "xgboost_multigraph_enhanced",
    ...
    metrics_path = (
        EVALUATION_DIR
        / "xgboost_multigraph_enhanced_validation.json"
    )
    ...
    print("\n" + "=" * 70)
    print("MULTIGRAPH XGBOOST COMPLETE")
    print("=" * 70)
